...
from stream_generators import StreamGenerator, FalconStreamGenerator
import utils.constants
from utils.load_quant import load_awq_model, load_awq_llama_fast
from utils.prompt_templates import get_prompter, get_stop_token_ids
from utils.tune import device_warmup, tune_all_wqlinears
...
if __name__ == "__main__":
    ...
    assert args.precision in ["W4A16", "W2A16", "W16A16"], "We only support W4A16/W2A16/W16A16 now"

    gen_params.n_predict = 512
    gen_params.n_vocab = 32000
    utils.constants.max_batch_size = args.max_batch_size
    utils.constants.max_seq_len = args.max_seq_len
    utils.constants.mem_efficient_load = args.mem_efficient_load
    if utils.constants.mem_efficient_load:
        print("=" * 80)
        print(
    ...
    from models import FalconForCausalLM, LlamaForCausalLM, MPTForCausalLM
    ...
    model_type_dict = {
        ...
        "mpt": MPTForCausalLM,
        "metamath": LlamaForCausalLM
    }

    if args.precision == "W4A16":
        if args.model_type.lower() == "llama" or args.model_type.lower() =="metamath":
    ...
    elif args.precision == "W2A16":
        if args.model_type.lower() == "llama" or args.model_type.lower() =="metamath":
            model = model_type_dict["llama"](config).half()
            model = load_awq_llama_fast(
                model, args.load_quant, 2, args.q_group_size, args.device
            )
        else:
            model = model_type_dict[args.model_type.lower()](config).half()
            model = load_awq_model(
    ...
    if args.precision == "W4A16" or args.precision == "W2A16" and (args.model_type.lower() == "llama" or args.model_type.lower() == "metamath"):
