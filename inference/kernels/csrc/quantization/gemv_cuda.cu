...
/*
Computes GEMV-2 bit (group_size = 128).

Args:
  inputs: vector of shape [batch_size, IC];
  weight: matrix of shape [OC, IC / 16];
  output: vector of shape [OC * B];
  zeros: matrix of shape [OC, IC / group_size / 16];
  scaling_factors: matrix of shape [OC, IC / group_size];
...
Notes:
  ...
  the second dimension is rounded up to a multiple of PACK_FACTOR.
*/
__global__ void gemv_kernel_g128_2bit(
  const float4* _inputs, const uint32_t* weight, 
  const uint32_t* zeros, const half* scaling_factors, half* _outputs, 
  const int IC, const int OC){
    const int group_size = 128;
    float psum = 0;
    const int batch_idx = blockIdx.z;
    const int oc_idx = blockIdx.y * blockDim.y + threadIdx.y; 
    const float4* inputs = _inputs + batch_idx * IC / PACK_FACTOR2;
    half* outputs = _outputs + batch_idx * OC;
    const int num_groups_packed = make_divisible(IC / group_size, PACK_FACTOR2);

    const int weight_w = IC / PACK_FACTOR2;
    // TODO (Haotian): zeros_w is incorrect, after fixing we got misaligned address
    const int zeros_w = make_divisible(IC / group_size, PACK_FACTOR2);
    // consistent with input shape
    const int sf_w = make_divisible(IC / group_size, PACK_FACTOR2) * PACK_FACTOR2;
    //if(blockIdx.x == 0 && blockIdx.y == 0 && threadIdx.x == 0 && threadIdx.y == 0) printf("%d %d %d %d\n", IC, group_size, PACK_FACTOR, zeros_w);
    // tile size: 4 OC x 2048 IC per iter
    for(int packed_group_idx = 0; packed_group_idx < num_groups_packed; packed_group_idx++) {
      // load input (float4)
      int inputs_ptr_delta = packed_group_idx * WARP_SIZE * 8 + threadIdx.x * 8; 
      // load zeros: 2048 numbers in one iteration across warp. Need 2048 / group_size zeros.
      uint32_t packed_zeros = *(zeros + oc_idx * zeros_w + packed_group_idx);
      float current_zeros = (float)((packed_zeros >> (threadIdx.x / 2 * 2)) & 0x03);
      

      // load weight: use float4 to load weights, each thread load 64 int2 numbers (1 x float4)
      uint32_t packed_weights[4];
      *((float4*)(packed_weights)) = *((float4*)(weight + oc_idx * weight_w + packed_group_idx * (WARP_SIZE * 4) + threadIdx.x * 4));

      // load scaling factors:
      // g128: four threads -> 128 numbers -> 1 group; 1 warp = 16 groups.
      float scaling_factor = __half2float(scaling_factors[oc_idx * sf_w + packed_group_idx * 16 + (threadIdx.x / 2)]);
      
      const float4* inputs_ptr = inputs + inputs_ptr_delta;
      uint32_t current_packed_weight = packed_weights[0];

      // multiply 64 weights with 64 inputs
      #pragma unroll
      for (int ic_0 = 0; ic_0 < 8; ic_0++){
        // iterate over different uint32_t packed_weights in this loop
        if (ic_0 % 2 == 0) {
          current_packed_weight = packed_weights[ic_0 / 2];
        }

        half packed_inputs[PACK_FACTOR2];
        // each thread load 16 inputs, starting index is packed_group_idx * 128 * 8 (because each iter loads 128*8)
        if (inputs_ptr_delta + ic_0 < ((packed_group_idx + 1) * (IC / PACK_FACTOR2))) {

          *((float4*)packed_inputs) = *(inputs_ptr + ic_0);
          #pragma unroll
          for (int ic_1 = 0; ic_1 < 8; ic_1++){
            // iterate over 16 numbers packed within each uint32_t number
            float current_single_weight_fp = (float)(current_packed_weight & 0x03);
            float dequantized_weight = scaling_factor * (current_single_weight_fp - current_zeros);
    ...
    int group_size)
{
    ...
    {
      if (w_bit == 2) {
        gemv_kernel_g128_2bit<<<num_blocks, num_threads>>>(
        // pointers
        in_feats, kernel, zeros, scaling_factors, out_feats,
        // constants
        num_in_channels, num_out_channels
